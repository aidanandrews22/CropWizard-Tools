...
# TODO: Allow matplotlib to create visualizations in s3
...
class URLHandler:
    ...
    def get_url_type(url: str) -> str:
        ...
        if (
            ...
            and "/spreadsheets/" in parsed_url.path
        ):
            return "google_sheet"
        ...
        # Check for S3 bucket URLs
        if parsed_url.scheme == "s3" or "s3.amazonaws.com" in parsed_url.netloc:
            if parsed_url.path.lower().endswith(".csv"):
                return "s3_csv_file"
...
class S3Handler:
    """Handler for S3 bucket URLs"""

    def __init__(self):
        import boto3
        from botocore.client import Config

        # Configure S3 client with reasonable timeouts
    ...
    def parse_s3_url(self, url: str) -> tuple:
        """
        Parse an S3 URL to extract bucket name and object key

        Args:
            url (str): S3 URL in format s3://bucket-name/key or https://bucket-name.s3.amazonaws.com/key

        Returns:
            tuple: (bucket_name, object_key)

        Raises:
            ValueError: If the URL cannot be parsed as an S3 URL
        """
        parsed_url = urlparse(url)
        ...
        # Handle https://bucket-name.s3.amazonaws.com/key format
        ...
        # Handle other potential S3 URL formats
        elif parsed_url.netloc.endswith(".s3.amazonaws.com"):
            bucket_name = parsed_url.netloc.split(".")[0]
            object_key = parsed_url.path.lstrip("/")
            return bucket_name, object_key

        raise ValueError(f"Could not parse S3 URL: {url}")
    ...
    def download_file(self, url: str, output_path: str) -> str:
        ...
        """
        try:
            bucket_name, object_key = self.parse_s3_url(url)

            logger.info(
                f"Downloading S3 file from bucket '{bucket_name}', key '{object_key}'"
            )
...
class GoogleSheetsConverter:
    ...
    def convert_to_csv(self, sheet_url: str, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            if url_type == "csv_file":
            ...
            elif url_type == "s3_csv_file":
                # Handle S3 CSV files
                logger.info(f"Downloading CSV from S3: {sheet_url}")
                ...
                # Use S3Handler to download the file
                s3_handler = S3Handler()
                s3_handler.download_file(sheet_url, temp_path)

                # Read the downloaded file
                df = pd.read_csv(temp_path)

                # If we created a temp file and no output path was specified, clean up
                if not output_path:
                    os.unlink(temp_path)
                ...
                for attempt in range(max_retries):
                    try:
                        response = self.session.get(
                            csv_export_url, timeout=self.timeout
                        )
                        ...
                        if attempt == max_retries - 1:
                            raise Exception(
                                f"Failed to download Google Sheet after {max_retries} attempts: {str(e)}"
                            )
                        logger.warning(
...
class CSVProcessor:
    ...
    def process_urls(self, urls, convert_func):
        ...
        for url in urls:
            try:
                ...
                results["success"].append(
                    {"url": url, "row_count": len(df), "column_count": len(df.columns)}
                )
                logger.info(
                    f"Successfully processed {url}: {len(df)} rows, {len(df.columns)} columns"
                )
        ...
        # Log summary
        logger.info(
            f"Processing complete: {len(results['success'])} succeeded, {len(results['failed'])} failed"
        )
...
class CSVAnalyzer:
    def __init__(self, query, processed_files):
        ...
        for file_info in processed_files:
            if (
                "df" in file_info
                and isinstance(file_info["df"], pd.DataFrame)
                and not file_info["df"].empty
            ):
        ...
        try:
            ...
            for idx, file_info in enumerate(self.processed_files):
                ...
                error_results[csv_path] = {
                    "data": {"input": self.query, "output": f"Error: {error_message}"},
                    "error": str(e),
                    "query": self.query,
    ...
    def _build_system_prompt(self):
        ...
        system_prompt += (
            "2. If multiple dataframes are provided, determine which are relevant\n"
        )
        ...
        system_prompt += (
            "5. If the query cannot be answered with the available data, explain why\n"
        )
        system_prompt += "6. You do not need to create any new dataframes, just use the ones provided\n"
    ...
    def _format_results(self, analysis_result):
        ...
        for idx, file_info in enumerate(self.processed_files):
            ...
            formatted_results[csv_path] = {
                "data": {"input": self.query, "output": analysis_result},
                "query": self.query,
                "url": file_info.get("url", "Local CSV"),
                "row_count": file_info.get(
                    "row_count", len(file_info.get("df", pd.DataFrame()))
                ),
                "column_count": file_info.get(
                    "column_count", len(file_info.get("df", pd.DataFrame()).columns)
                ),
            }
...
def main(**inputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Analyzes CSV data from Google Sheets, direct CSV URLs, or S3 bucket URLs based on a user query.

    Args:
        google_sheets (Optional[List[str]]): List of Google Sheet, CSV, or S3 bucket URLs to analyze
    ...
    try:
        ...
        for url in google_sheets:
            ...
            if url_type == "invalid":
                ...
                invalid_urls.append(
                    {
                        "url": url,
                        "error": "URL is not a recognized Google Sheet, CSV file, or S3 URL",
                    }
                )
            else:
        ...
        try:
            ...
            # Process the URLs
            processed_files = processor.process_urls(
                valid_urls, converter.convert_to_csv
            )
