...
ENDPOINT_URL = "https://3be6faac-2c21-4696-96d5-315f4f2140d0.app.beam.cloud"
...
def call_endpoint(query, google_sheets=None):
    ...
    try:
        ...
        headers_json = {"Content-Type": "application/json", "Authorization": AUTH_TOKEN}

        response_json = requests.post(
            ENDPOINT_URL, json=payload, headers=headers_json, timeout=120
        )
        ...
        response_form = requests.post(
            ENDPOINT_URL, data=payload, headers=headers_form, timeout=120
        )
        ...
        response_get = requests.get(
            ENDPOINT_URL, params=payload, headers=headers_json, timeout=120
        )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            'Usage: python call_endpoint.py "Your query here" [google_sheet_url1] [google_sheet_url2] ...'
        )
        return
